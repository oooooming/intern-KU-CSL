...
#define BLANK_CYCLE	"   "
#define NUM_STAGES	9
const char *stage_str[NUM_STAGES] = {"FE ", "DE ", "RN ", "DI ", "IS ", "RR ", "EX ", "WB ", "CM "};
...
class printline {
	...
	public:
		...
		void print(char *line) {
		   ...
		   scan = sscanf(line, "%d fu{%d} src{%d,%d} dst{%d} FE{%d,%d} DE{%d,%d} RN{%d,%d} DI{%d,%d} IS{%d,%d} RR{%d,%d} EX{%d,%d} WB{%d,%d} CM{%d,%d}",
			&seq_no,
			&fu_type,
			&src1, &src2,
			&dst,
		   ...
		   if (scan != 23) {
		      ...
		      fprintf(stderr, "If you cannot determine the problem, contact `heysid@ajou.ac.kr'.\n");
		      exit(-1);
		   }
